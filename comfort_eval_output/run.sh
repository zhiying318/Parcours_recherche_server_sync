...
# Single-image MCQ on COMFORT comfort_human_car
# Each query: 1 image, 4-choice spatial relation MCQ
# (short / middle / long answer lengths).
#
# Expected image list:
#   144 images total = 9 objects x 4 relations x 4 camera views
#
# Step 0 (one-time): regenerate image list if dataset changes
#   cd /home/zzou
#   python comfort_eval_output/get_comfort_image_paths_json.py
#
# Step 1: run evaluation (below) run directly
#   ./comfort_eval_output/run.sh

IMAGES_JSON="$(dirname "$0")/../comfort_image_paths.json"
...
run_gpu0_queue() {
  ...
  #   --model_id google/gemma-4-E4B-it \
  #   --image_json "$IMAGES_JSON" \
  #   --out_csv "$RESULTS_DIR/mcq_short_gemma4_thinking.csv" \
  #   --ask_mode mcq \
  #   --answer_length short \
  #   --enable_thinking \
  ...
  #   --model_id google/gemma-4-E4B-it \
  #   --image_json "$IMAGES_JSON" \
  #   --out_csv "$RESULTS_DIR/mcq_middle_gemma4_thinking.csv" \
  #   --ask_mode mcq \
  #   --answer_length middle \
  #   --enable_thinking \
  #   --max_new_tokens_mcq 81920 \
